...
public class Item
{
    ...
    public byte HandsCount() => gbl.ItemDataTable[type].handsCount;

    public bool ScrollLearning(int i, int spell) => (int)getAffect(i) > 0x7F && ((int)getAffect(i) & 0x7F) == spell;

    public bool IsScroll() => gbl.ItemDataTable[type].item_slot >= ItemSlot.slot_11 &&
                              gbl.ItemDataTable[type].item_slot <= ItemSlot.slot_13;
    ...
    public string GenerateName(int hidden_names_flag)
    {
        ...
        var pural_added = false;
        var name = "";

        for (var var_1 = 3; var_1 >= 1; var_1--)
        {
            if (((display_flags >> (var_1 - 1)) & 1) > 0)
            {
                    ...
                    pural_added == true)
                {
                    name += " ";
                         ...
                         (namenum3 != 0x87 &&
                          (type == ItemType.Arrow || type == ItemType.Quarrel || type == ItemType.Dart) &&
                          namenum3 != 0xb1))
                {
                    name += "s ";
                    pural_added = true;
    ...
    public override string ToString() => GenerateName(0);


    public bool IsRanged() => gbl.ItemDataTable[type].range > 1;
}
