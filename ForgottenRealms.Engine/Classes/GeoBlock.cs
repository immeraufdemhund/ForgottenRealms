...
public class GeoBlock
{
    ...
    {
        data = new byte[0x400];
        Array.Copy(_data, 2, data, 0, 0x400);
...
public class WallDefs
{
    ...
    {
        var offset = 0;
        for (var i = 0; i < _data.Length / 780; i++)
        {
    ...
    public void BlockOffset(int set, int offset) => blocks[set - 1].Offset(offset);
}

public class WallDefBlock
{
    private byte[,] data = new byte[5, 156];
    ...
    public int Id(int y, int x) => data[y, x];
...
public class MapInfo
{
    ...
    internal MapInfo(byte[] data, int map_x, int map_y)
    {
        ...
        var b = data[0x300 + map_y_x16 + map_x];
