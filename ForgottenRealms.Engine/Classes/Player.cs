...
{
    private int[,,] raceSexMinMax;
    private int[] classMin;
    private int[] ageEffects;
    ...
    public void Load(int val) => full = cur = val;
    ...
    public override string ToString() => string.Format("{0}/{1}", cur, full);
}

public class PlayerStats : IDataIO
{
    public StatValue Str = new(Limits.StrRaceSexMinMax, Limits.StrClassMin, Limits.StrAgeEffect);
    public StatValue Str00 = new(Limits.Str00RaceSexMinMax, Limits.Str00ClassMin, Limits.Str00AgeEffect);
    public StatValue Con = new(Limits.ConRaceSexMinMax, Limits.ConClassMin, Limits.ConAgeEffect);
    public StatValue Dex = new(Limits.DexRaceSexMinMax, Limits.DexClassMin, Limits.DexAgeEffect);
    public StatValue Int = new(Limits.IntRaceSexMinMax, Limits.IntClassMin, Limits.IntAgeEffect);
    public StatValue Wis = new(Limits.WisRaceSexMinMax, Limits.WisClassMin, Limits.WisAgeEffect);
    public StatValue Cha = new(Limits.ChaRaceSexMinMax, Limits.ChaClassMin, Limits.ChaAgeEffect);
    ...
    public override string ToString() =>
        string.Format("S:{0} ({6}),I:{1},W:{2},C:{3},D:{4},Ch:{5}", Str, Int, Wis, Con, Dex, Cha, Str00);
}
...
{
    private const int ItemSlots = 13;
    private Item[] itemArray = new Item[ItemSlots]; // 0x151[]
    ...
    {
        get => itemArray[(int)slot];
        set => itemArray[(int)slot] = value;
    }
    ...
    {
        for (var item_slot = 0; item_slot < ItemSlots; item_slot++)
        {
...
public class Player
{
    ...
    public bool KnowsSpell(Spells spell) => spellBook[(int)spell - 1] != 0;
    ...
    public byte[]
        thief_skills =
            new byte[8]; // 0xeA; [] was 1 offset @ 0xe9, pick_pockets, open_locks, find_remove_traps, move_silently, hide_in_shadows, hear_noise, climb_walls, read_languages
    ...
    public ActiveItems activeItems = new();
    ...
    public bool CanDuelClass()
    {
        ...
        for (var index = ClassId.cleric; index <= ClassId.monk; index++)
        {
    ...
    private int DualClassExceedsPreviousLevel() // sub_6B3D1
    {
    ...
    private int DuelClassCurrentLevel()
    {
        if (race != Race.human)
        {
            return 0;
    ...
    public CombatTeam OppositeTeam() => combat_team == CombatTeam.Ours ? CombatTeam.Enemy : CombatTeam.Ours;

    public bool HasAffect(Affects type) => affects.Exists(aff => aff.type == type);

    public Affect GetAffect(Affects type) => affects.Find(aff => aff.type == type);

    private static Affects[] held_affects = { Affects.snake_charm, Affects.paralyze, Affects.sleep, Affects.helpless };

    public bool IsHeld() => Array.Exists(held_affects, affect => HasAffect(affect));


    public void RemoveWeight(int amount) => weight -= (short)amount;


    public void AddWeight(int amount) => weight += (short)amount;
}
