...
{
    private DaxBlock normal = null;
    private DaxBlock attack = null;

    // cheat and cache the flipped copy, so we don't damage the original, then we present this one later.
    private DaxBlock normal_f = null;
    private DaxBlock attack_f = null;
    ...
    {
        var data = DaxCache.LoadDax(file_text + ".dax", block_id);
    ...
    {
        var bitPerPixel = normal.bpp;

        Array.Copy(combatIcon.normal.data, normal.data, combatIcon.normal.data.Length);
        Array.Copy(combatIcon.normal_f.data, normal_f.data, combatIcon.normal_f.data.Length);
        Array.Copy(combatIcon.attack.data, attack.data, combatIcon.attack.data.Length);
        Array.Copy(combatIcon.attack_f.data, attack_f.data, combatIcon.attack_f.data.Length);

        if (Recolour)
        {
            var newColors = new byte[16];
            var oldColors = new byte[16];
