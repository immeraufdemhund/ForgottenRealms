...
public class SpellItem
{
    ...
    public SpellItem(int id, bool learning)
    {
        Id = id;
        Learning = learning;
    }
...
public class SpellList
{
    ...
    private List<SpellItem> spells = new();
    ...
    public void AddLearn(int id) => spells.Add(new SpellItem(id, true));

    public void AddLearnt(int id) => spells.Add(new SpellItem(id & 0x7F, id > 0x7f));
    ...
    public bool HasSpell(int id) => spells.Exists(sp => sp.Id == id);

    public void CancelLearning() => spells.RemoveAll(sp => sp.Learning == true);

    public void Load(byte[] data, int offset)
    {
        for (var i = 0; i < SpellListSize; i++)
        {
            if (data[offset + i] > 0)
            {
