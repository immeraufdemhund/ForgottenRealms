...
public class ovr025
{
    ...
    public ovr025(DisplayDriver displayDriver, KeyboardDriver keyboardDriver, ovr020 ovr020, ovr027 ovr027, ovr029 ovr029, ovr030 ovr030, ovr032 ovr032, ovr033 ovr033, seg037 seg037, seg040 seg040, seg051 seg051, SoundDriver soundDriver)
    {
    ...
    internal void CalculateAttackValues(Player player) // sub_66023
    {
    ...
    internal void CalcArmorWeightEffect(Item item, Player player) // sub_6621E
    {
    ...
    internal void sub_662A6(ref byte output, ref sbyte[] bonus, Item item, Player player)
    {
    ...
    internal void calc_movement(Player player) /* sub_663C4 */
    {
    ...
    internal void ItemDisplayNameBuild(bool display_new_name, bool displayReadied,
        int yCol, int xCol, Item item) /*id_item*/
    {
        ...
        if (display_new_name)
        {
            _displayDriver.displayString(item.name, 0, 10, yCol, xCol);
        }
    }


    internal void PartySummary(Player player)
    {
        ...
        _displayDriver.displayString("Name", 0, 15, y_pos, x_pos);
        _displayDriver.displayString("AC  HP", 0, 15, y_pos, 0x21);

        y_pos += 2;

        foreach (Player tmp_player in gbl.TeamList)
        {
            _seg037.draw8x8_clear_area(y_pos, 0x26, y_pos, x_pos);

            if (tmp_player == player)
            {
                _displayDriver.displayString(tmp_player.name, 0, 15, y_pos, x_pos);
            }
            else
            {
            ...
            _displayDriver.displayString(string.Format("{0,-3}", tmp_player.DisplayAc), 0, 10, y_pos, 0x1F);
    ...
    internal void display_AC(int y_offset, int x_offset, Player player)
    {
        _displayDriver.displayString(player.DisplayAc.ToString(), 0, 10, y_offset, x_offset);
    }


    internal void display_hp(bool hightlighted, int y_pos, int x_pos, Player player)
    {
        int colour;
        ...
        _displayDriver.displayString(player.hit_point_current.ToString(), 0, colour, y_pos, x_pos);
    }


    internal void CombatDisplayPlayerSummary(Player player) /* hitpoint_ac */
    {
        if (gbl.display_hitpoints_ac == true)
        {
            ...
            _displayDriver.displayString("Hitpoints", 0, 10, line + 1, 0x17);

            display_hp(false, line + 1, 0x21, player);
            line += 2;

            _displayDriver.displayString("AC", 0, 10, line + 1, 0x17);
            ...
            if (player.activeItems.primaryWeapon != null)
            {
                ...
                _displayDriver.press_any_key(player.activeItems.primaryWeapon.name, true, 10, line + 3, 0x26, line + 1, 0x17);
            }
            ...
            {
                _displayDriver.displayString(_ovr020.statusString[(int)player.health_status], 0, 15, line + 1, 0x17);
            }
            else if (player.IsHeld() == true)
            {
                _displayDriver.displayString("(Helpless)", 0, 15, line + 1, 0x17);
            }
        }
    }


    internal void reclac_player_values(Player player) /* sub_66C20 */
    {
        ...
        stat_bonus[0] = DexAcBonus(player);
    ...
    internal sbyte DexAcBonus(Player player) /* stat_bonus */
    {
        sbyte bonus;
    ...
    internal int DexReactionAdj(Player player)
    {
        int bonus;
    ...
    internal int player_strength_group(Player player) /* playerStrengh */
    {
        int ret_val;
    ...
    internal int strengthHitBonus(Player player)
    {
        int str_bonus = 0;
    ...
    internal sbyte strengthDamBonus(Player player)
    {
        int damage_bonus = 0;
    ...
    internal int max_encumberance(Player player) /* strength_bonus */
    {
    ...
    internal void lose_item(Item item, Player player)
    {
        if (!player.items.Remove(item))
        {
            _displayDriver.DisplayAndPause("Tried to Lose item & couldn't find it!", 14);
        }
    }


    internal void string_print01(string text)
    {
        _ovr027.ClearPromptAreaNoUpdate();

        _displayDriver.displayString(text, 0, 10, 0x18, 0);
    ...
    internal void DisplayPlayerStatusString(bool clearDisplay, int lineY, string text, Player player) /* sub_67788 */
    {
        if (gbl.game_state == GameState.Combat)
        {
            _seg037.draw8x8_clear_area(0x15, 0x26, lineY, 0x17);

            displayPlayerName(false, lineY, 0x17, player);
            _displayDriver.press_any_key(text, true, 10, 0x15, 0x26, lineY + 1, 0x17);
        }
        else
        {
            int line_y = gbl.displayPlayerStatusLine18 ? 18 : 17;

            _seg037.draw8x8_clear_area(0x16, 0x26, line_y, 1);

            displayPlayerName(false, line_y + 1, 1, player);
            _displayDriver.press_any_key(text, true, 10, 0x16, 0x26, line_y + 2, 1);
        }
    ...
    internal void ClearPlayerTextArea() /* sub_6786F */
    {
        if (gbl.game_state == GameState.Combat)
        {
            _seg037.draw8x8_clear_area(0x15, 0x26, 0x0a, 0x17);
        }
        else
        {
    ...
    internal void displayPlayerName(bool pural, int y_offset, int x_offset, Player player) /*sub_678A2*/
    {
        int color;
        ...
        _displayDriver.displayString(name, 0, color, y_offset, x_offset);
    }


    internal void load_missile_dax(bool flipIcon, byte iconOffset, Icon iconAction, int iconIdx) /* sub_67924 */
    {
    ...
    internal void load_missile_icons(int iconIdx) /* sub_67A59 */
    {
    ...
    internal void draw_missile_attack(int delay, int frameCount, Point target, Point attacker) /* sub_67AA4 */
    {
        Point center;
        bool var_B4;
        bool var_B3;
        ...
        _seg051.FillChar(8, 0x94, pathDir);
        ...
        if (_ovr033.CoordOnScreen(attacker - gbl.mapToBackGroundTile.mapScreenTopLeft) == false ||
            _ovr033.CoordOnScreen(target - gbl.mapToBackGroundTile.mapScreenTopLeft) == false)
        {
        ...
        _ovr033.redrawCombatArea(8, 0xFF, center);
        var_AF = 0;
        ...
        do
        {
            ...
            do
            {
                ...
                {
                    Display.SaveVidRam();
                    _seg040.OverlayBounded(gbl.missile_dax, 5, frame, cur.y, cur.x);
                    _seg040.DrawOverlay();

                    _keyboardDriver.SysDelay(delay);
            ...
            else
            {
                ...
                if (_ovr033.CoordOnScreen(target - gbl.mapToBackGroundTile.mapScreenTopLeft) == false)
                {
                    _ovr033.redrawCombatArea(8, 3, target);
                }
    ...
    internal void MagicAttackDisplay(string text, bool showMagicStars, Player player) // sub_6818A
    {
        if (gbl.game_state == GameState.Combat)
        {
            ...
            if (_ovr033.PlayerOnScreen(true, player) == false)
            {
                _ovr033.redrawCombatArea(8, 3, _ovr033.PlayerMapPos(player));
            }

            if (showMagicStars)
            {
                _soundDriver.PlaySound(Sound.sound_4);
            }
            else
            {
                _soundDriver.PlaySound(Sound.sound_3);
            }

            DisplayPlayerStatusString(false, 10, text, player);

            int idx = _ovr033.GetPlayerIndex(player);
            ...
            for (int loop = 0; loop <= loops; loop++)
            {
                for (int frame = 0; frame <= 3; frame++)
                {
                    ...
                    _seg040.OverlayBounded(gbl.missile_dax, 5, frame, pos.y, pos.x);
                    _seg040.DrawOverlay();

                    _keyboardDriver.SysDelay(70);
    ...
    internal bool FindAffect(out Affect affectFound, Affects affect_type, Player player)
    {
    ...
    internal void damage_player(int damage, Player player)
    {
        int neg_hp = 0;
        int new_hp = 0;
    ...
    internal void DescribeHealing(Player player) /* sub_684F7 */
    {
        string text;
    ...
    internal void CountCombatTeamMembers() // count_teams
    {
        gbl.friends_count = 0;
        gbl.foe_count = 0;
    ...
    internal List<CombatPlayerIndex> BuildNearTargets(int max_range, Player player) /*near_enermy*/
    {
        var scl = _ovr032.Rebuild_SortedCombatantList(player, max_range, p => p.combat_team != player.combat_team);
    ...
    internal int getTargetRange(Player target, Player attacker) /* sub_68708 */
    {
        ...
        var scl = _ovr032.Rebuild_SortedCombatantList(attacker, 0xff, p => p == target);
    ...
    internal void clear_actions(Player player)
    {
        player.actions.Clear();
    }


    internal void guarding(Player player)
    {
        player.actions.Clear();
    ...
    internal int spellMaxTargetCount(int spell_id) /* sub_6886F */
    {
        int target_count = 0;
    ...
    internal void LoadPic() // load_pic
    {
        ...
        switch (gbl.game_state)
        {
            ...
            case GameState.Shop:
                ...
                {
                    _ovr030.head_body(gbl.body_block_id, gbl.head_block_id);
            ...
            case GameState.Camping:
                _seg037.draw8x8_03();
                _ovr030.load_pic_final(ref gbl.byte_1D556, 0, 0x1d, "PIC");
                PartySummary(gbl.SelectedPlayer);
                display_map_position_time();
                break;
            ...
            case GameState.AfterCombat:
                _seg037.draw8x8_03();
                _ovr030.load_pic_final(ref gbl.byte_1D556, 0, 1, "PIC");
                PartySummary(gbl.SelectedPlayer);
                break;
        }
    }
    ...
    internal void display_map_position_time() // camping_search
    {
        if (gbl.game_state != GameState.WildernessMap)
        {
            ...
            _seg037.draw8x8_clear_area(15, 0x26, 15, 17);

            _displayDriver.displayString(output, 0, 10, 15, 17);
        }
    }


    internal void RedrawCombatScreen() // sub_68DC0
    {
        _ovr033.Color_0_8_inverse();
        _seg037.DrawFrame_Combat();

        _ovr033.redrawCombatArea(8, 0xff, gbl.mapToBackGroundTile.mapScreenTopLeft + Point.ScreenCenter);
    }

    private Set unk_68DFA = new Set(13, 27, 69, 83);


    internal void selectAPlayer(ref Player player, bool showExit, string prompt)
    {
        ...
        while (unk_68DFA.MemberOf(input_key) == false)
        {
            ...
            input_key = _ovr027.displayInput(out special_key, useOverlay, 1, gbl.defaultMenuColors, "Select" + text, prompt + " ");
    ...
    internal bool item_is_ranged_melee(Item item)
    {
    ...
    internal bool is_weapon_ranged(Player player) /* offset_above_1 */
    {
        return player.activeItems.primaryWeapon != null && player.activeItems.primaryWeapon.IsRanged();
    }


    internal bool is_weapon_ranged_melee(Player player) /* offset_equals_20 */
    {
        return item_is_ranged_melee(player.activeItems.primaryWeapon);
    }


    internal bool GetCurrentAttackItem(out Item found_item, Player player) // sub_6906C
    {
        found_item = null;
    ...
    internal bool bandage(bool applyBandage)
    {
